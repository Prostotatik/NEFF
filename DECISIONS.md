...
is measuring *verifier independence* at query time. Product name at the time: **Quorum** — a quorum is the number
...
(text/images)". NEFF handles text only, and that is not a scoping choice — the Gonka Router rejects
...
generated by a centralized server". NEFF satisfies the section 3 definition — every inference shows
...
## D21 — Renamed to NEFF (2026-09-05)
The product was called Quorum. It is now **NEFF**, and the rename does three things at once.

**It names the quantity instead of the metaphor.** A quorum is the number of members a decision needs
to be valid — a good gloss, but a gloss. `N_eff` is the effective sample size: how much independent
information a set of correlated observations actually carries. That is not an analogy for what this
app computes, it is literally the number on the screen. The estimator is Kish's, and it has already
been applied to an LLM panel in *Nine Judges, Two Effective Votes* (arXiv:2605.29800), which measures
nine frontier models as carrying about two independent votes because their errors correlate. Nine
judges are not nine opinions; NEFF is the number that says how many they are worth.

**It resolves a name collision that `PRIOR_ART.md` had flagged as a submission risk.** Four shipped or
near-shipped products in this exact space are called some form of "Quorum" — PromptQuorum, Quorum
(aiquorum.app), AskQuorum AI, and `qinnovates/quorum`, the last of which also prints an independence
score. A judge searching the old name met them first. NEFF collides with none of them, and searching
it leads to the statistical concept the product implements.

**It avoids the other failure mode**, which is the generic one: TruthAI, FactCheck, VerifyAI. NEFF is
short, technical, and means something specific to anyone who recognises it — and to anyone who does
not, the footer says it in one line: *N eff is how many independent voices are really behind an
agreement.*

The claim the name makes is deliberately narrow, and it is the honest one for this product: NEFF does
not claim to know the truth. It measures how much the consensus is worth.
